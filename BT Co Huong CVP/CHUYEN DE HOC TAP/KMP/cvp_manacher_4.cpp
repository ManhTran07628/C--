...
bool check(int pos1,int pos2)
{
    ...
    int L1 = 1, R1 = len;
    int R2 = n - pos2, L2 = 1;
    len = min(R2 - L2 + 1,R1 - L1 + 1);
    // cout << L1 << ' ' << L1 + len - 1 << ' ' << R2 - len + 1 << ' ' << R2 << '\n';
    if(getkey(R1,R1 - len + 1,HashS) == getkey(R2 - len + 1,R2,HashT)) return 1;
    return 0;
}
...
signed main()
{
    ...
    int max_L = 1;
    for(int L = 1;L <= n;L++) {
        if(getkey(n - L + 1, n, HashS) == getkey(1, L, HashT)) {
            max_L = L;
        }
    }
    ...
    for(int i = max_L + 1;i <= n;i++) cout << t[i];
